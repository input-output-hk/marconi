...
-- | TODO: PLT-8076 borrow what you can from chain-index Defines REST and JSON-RPC routes
module Marconi.Sidechain.Experimental.Api.Routes where
...
import Marconi.Sidechain.Experimental.CLI (CliArgs)
...
{- | TODO: PLT-8098 note this is a non-standard conversion,
To get it generically, we will need to provide Maybe
(some record with those fields), where Nothing represents
ChainTipAtGenesis.
-}
...
instance FromJSON SidechainTip where
  ...
  parseJSON obj =
    let parseTip v = do
          ...
          bn <- v .: "blockNo"
          pure $ SidechainTip $ C.ChainTip sn bhh bn
     in Aeson.withObject "ChainTip" parseTip obj

-- TODO: PLT8076 another holderover from the old sidechain
-- that we need to support. will want to use the chain-index route
-- but transform from this one.
...
instance ToJSON GetCurrentSyncedBlockResult where
  toJSON (GetCurrentSyncedBlockResult blockInfoM tip) =
    let nodeTip = case tip of
        ...
        chainPointObj =
          case blockInfoM of
            -- TODO: PLT-8076 BlockInfo in legacy in new are not the same
...
-- TODO: PLT-8076 chain-index no longer exports an Interval
-- so we need to create one in this module that does the same stuff.
-- likely need to revise this based on the usage hree.
-- Taken from chain-index-legacy but likely should be moved
...
data GetUtxosFromAddressParams = GetUtxosFromAddressParams
  ...
  , querySearchInterval :: !(Interval Ledger.SlotNo)
  -- ^ Query interval
  }
  deriving stock (Show, Eq)

-- | TODO: PLT-8076
interval :: Maybe Ledger.SlotNo -> Maybe Ledger.SlotNo -> Either () (Interval Ledger.SlotNo)
interval = undefined

-- TODO: PLT-8076 reshape to use generic aeson instances
  ...
  parseJSON =
    let buildInterval v = do
        ...
        parseParams v = do
          ...
          iv <- buildInterval v
          pure $ GetUtxosFromAddressParams address iv
     in Aeson.withObject "GetUtxosFromAddressParams" parseParams

-- TODO: PLT-8076 reshape types to use derived aeson
instance ToJSON GetUtxosFromAddressParams where
  toJSON q =
    Aeson.object $
      catMaybes
        [ Just ("address" .= queryAddress q)
        , ("createdAtOrAfterSlotNo" .=) <$> (intervalLowerBound $ querySearchInterval q)
        , ("unspentBeforeSlotNo" .=) <$> (intervalUpperBound $ querySearchInterval q)
        ]
