...
endToEndMintTokenEvent = Helpers.unitTestWithTmpDir "." $ \tempPath -> do
  ...
  res <- H.evalIO
    $ Async.race
      (Runner.runIndexerOnChainSync config coordinator)
    $ do
...
stabilityTestBase assertion upperSlotNo securityParam = H.property $ do
  ...
  let blockEvents = Test.BlockInfo.getTimedBlockInfoEventsFixedBlockNos events
...
indexMockchain
  ...
  chainWithInfo = do
    ...
    Concurrent.modifyMVar_
      blockInfoVar
      ( withGen
          (pure $ Test.BlockInfo.getTimedBlockInfoEventsFixedBlockNos chainWithInfo)
      )
