...
propUtxoQueryShouldRespondWithResolvedDatums = Hedgehog.property $ do
  let datGen =
        Gen.choice
          [ pure Gen.NoDatumLocation
          , fmap (\d -> Gen.TxOutDatumHashLocation (C.hashScriptDataBytes d)) CGen.genHashableScriptData
          ...
          , fmap (\d -> Gen.PlutusScriptDatumLocation (C.hashScriptDataBytes d) d) CGen.genHashableScriptData
          ]
  addrsWithDatum <- forAll $ Gen.genAddressesWithDatum datGen
  txs <- forAll $ Gen.genTxsWithAddresses addrsWithDatum
  ...
  let blockInfo = BlockInfo slotNo bhh (C.BlockNo sn) 0 1
      ...
      utxoEvents = fmap (\tx -> Utxo.getUtxoEvents utxoIndexerConfig [tx] blockInfo) txs
  ...
  let expectedDatumHashes = Set.fromList $ mapMaybe (Gen.getDatumHashFromDatumLocation . snd) addrsWithDatum
  let expectedDatums = Set.fromList $ mapMaybe (fmap C.getScriptData . Gen.getDatumFromDatumLocation . snd) addrsWithDatum
