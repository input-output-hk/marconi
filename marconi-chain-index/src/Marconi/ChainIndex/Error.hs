...
liftSQLError errorWrapper = liftCatch
    ( `catch` \(err0 :: SQL.SQLError) -> pure (Left (errorWrapper $ Text.pack $ show err0))
      `catch` \(err1 :: SQL.FormatError) -> pure (Left (InvalidIndexer $ Text.pack $ show err1))
