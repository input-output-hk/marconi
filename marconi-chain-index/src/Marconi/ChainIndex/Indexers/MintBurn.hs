...
toUpdate :: Maybe (NonEmpty (C.AssetName, C.PolicyId)) -> C.BlockInMode C.CardanoMode -> Maybe TxMintEvent
...
txMints :: Maybe (NonEmpty (C.AssetName, C.PolicyId)) -> TxIndexInBlock -> C.Tx era -> Maybe TxMintInfo
