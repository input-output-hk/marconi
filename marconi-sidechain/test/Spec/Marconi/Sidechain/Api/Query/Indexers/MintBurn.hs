...
import Spec.Marconi.Sidechain.RpcClientAction (RpcClientAction, insertMintBurnEventsAction, mocMintBurnWorker,
                                               queryMintBurnAction)
import Test.Tasty (TestTree, testGroup)
import Test.Tasty.Hedgehog (testPropertyNamed)

tests :: RpcClientAction -> TestTree
tests rpcClientAction = testGroup "marconi-sidechain-minting-policy query Api Specs"
    [ testPropertyNamed
...
queryMintingPolicyTest = property $ do
  ...
  let
    callback :: MintBurn.MintBurnIndexer -> IO ()
    callback = atomically
