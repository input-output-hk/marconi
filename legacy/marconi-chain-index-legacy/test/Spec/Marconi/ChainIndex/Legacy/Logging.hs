...
module Spec.Marconi.ChainIndex.Legacy.Logging (tests) where
...
tests =
  testGroup
    ...
    [ goldenVsStringDiff
        ...
        "test/Spec/Marconi/ChainIndex/Legacy/Logging/Golden/start-from-genesis-logging.txt"
        goldenStartFromGenesisLogging
    , goldenVsStringDiff
        "Golden test for syncing logs when starting from chain point other than genesis"
        (\expected actual -> ["diff", "--color=always", expected, actual])
        "test/Spec/Marconi/ChainIndex/Legacy/Logging/Golden/start-from-later-point-logging.txt"
