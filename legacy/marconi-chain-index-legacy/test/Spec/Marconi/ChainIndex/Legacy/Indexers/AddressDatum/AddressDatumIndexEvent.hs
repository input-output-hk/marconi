...
module Spec.Marconi.ChainIndex.Legacy.Indexers.AddressDatum.AddressDatumIndexEvent (
  tests,
) where
...
import Spec.Marconi.ChainIndex.Legacy.Indexers.AddressDatum.Generators (
  genTxBodyContentWithPlutusScripts,
 )
import Spec.Marconi.ChainIndex.Legacy.Indexers.AddressDatum.Utils (addressInEraToAddressAny)

tests :: TestTree
tests =
  localOption (HedgehogTestLimit $ Just 200) $
    testGroup
      "Spec.Marconi.Index.AddressDatum.AddressDatumIndexEvent.toAddressDatumIndexEvent"
      [ testPropertyNamed
...
propShouldIndexAddressWithTxOutDatum = property $ do
  ...
  Hedgehog.cover 30 "At least one address with datum hash in tx out" $
    isJust $
      List.find
        (\(_, dat) -> case dat of TxOutDatumHashLocation{} -> True; _ -> False)
        addressesDatum
  Hedgehog.cover 30 "At least one address with datum hash in tx out and datum in tx body" $
    isJust $
      List.find
        (\(_, dat) -> case dat of TxOutDatumInTxLocation{} -> True; _ -> False)
        addressesDatum
  Hedgehog.cover 30 "At least one address with inline datum in tx out" $
    isJust $
      List.find
        (\(_, dat) -> case dat of TxOutDatumInlineLocation{} -> True; _ -> False)
        addressesDatum
          ...
          mapMaybe
            ( \(addr, datLoc) ->
                fmap (\(dh, _) -> (addressInEraToAddressAny addr, Set.singleton dh)) $
                  getDatumFromTxOutLocation datLoc
            )
