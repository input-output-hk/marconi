...
-}
module Marconi.Core.Indexer.MixedIndexer (
  Flushable (..),
  MixedIndexer,
  mkMixedIndexer,
...
import Marconi.Core.Indexer.ListIndexer (ListIndexer, events, mkListIndexer)
...
import Marconi.Core.Type (Point, Timed, point)
...
instance (IsSync event m mem, Monad event) => IsSync event m (MixedIndexer store mem) where
  ...
  lastStablePoint = lastStablePoint . view inMemory
