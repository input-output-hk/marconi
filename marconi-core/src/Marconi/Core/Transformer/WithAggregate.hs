...
-}
module Marconi.Core.Transformer.WithAggregate (
  WithAggregate,
  config,
  withAggregate,
...
import Marconi.Core.Transformer.Class (IndexerMapTrans (ConfigMap, unwrapMap, wrapMap))
...
newtype AggregateConfig output input = AggregationConfig
...
instance IndexerMapTrans WithAggregate where
  type ConfigMap WithAggregate = AggregateConfig
...
instance
  ...
  => Resetable m event (WithAggregate indexer output)
  where
