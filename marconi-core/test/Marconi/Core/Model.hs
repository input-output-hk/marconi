...
data IndexView a = IndexView
  ...
  , ixView :: a
  , ixSize :: Int
  -- ^ Size represents the stored history elements
  }
...
prop_rewindDepth c (ObservedBuilder ix) =
  let v = fromJust $ view ix
   in ixDepth v >= 2
        ==> forAll
          ( frequency
              [ (20, chooseInt (ixDepth v, ixDepth v * 2))
        ...
        $ \depth ->
          cover 15 (depth > ixDepth v) "Depth is larger than max depth."
            $ cover
              15
              (depth <= ixDepth v && depth > ixSize v)
...
prop_insertRewindInverse
  :: forall e a n m
   . (Monad m, Show e, Arbitrary e, Eq a)
  => Conversion m a e n
  -> ObservedBuilder a e n
  -> Property
prop_insertRewindInverse c (ObservedBuilder ix) =
  let v = fromJust $ view ix
   in -- rewind does not make sense for lesser depths.
      ixDepth v >= 2 ==>
...
prop_insertRewindNotifications
  :: forall e a n m
   . (Monad m, Show e, Arbitrary e, Eq n)
  => Conversion m a e n
  -> ObservedBuilder a e n
  -> Property
...
instance
  ...
  => Arbitrary (ObservedBuilder a e n)
  where
  arbitrary = sized $ \n -> do
    depth <-
      frequency
...
instance
  ...
  => Arbitrary (GrammarBuilder a e n)
  where
  arbitrary = sized $ \n -> do
    depth <-
      frequency
